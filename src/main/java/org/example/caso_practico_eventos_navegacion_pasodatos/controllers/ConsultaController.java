...
public class ConsultaController {
    ...
    void manejarClicTabla(MouseEvent event) {
        if (event.getClickCount() == 2) {
            ...
            if (seleccionado != null) mostrarDetalleCliente(seleccionado);
        }
    }

    private void mostrarDetalleCliente(Cliente cliente) {
        try {
            SceneManager.<DetalleClienteController>abrirVentanaModal(
                    "/org/example/caso_practico_eventos_navegacion_pasodatos/DetalleCliente-view.fxml",
                    "Detalle del Cliente",
                    controller -> controller.setCliente(cliente)
            );
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private Button crearBotonAccion(String texto, String ayuda) {
        ...
        boton.setMinWidth(32); boton.setMinHeight(28);
    ...
    private void actualizarConteoRegistros() {
        if (lblRegistrosEncontrados != null && tblClientes != null) {
